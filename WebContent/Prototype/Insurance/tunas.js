...
function OpenWinViewPartialRequestNoNew(pStyle,pApprovalNo)
			{				
				window.open(ServerName + App +  '/AccMnt/PartialAssetPrepayNew/PartAssetPrepayViewNew.aspx?style=' + pStyle + '&ApprovalNo=' + pApprovalNo, 'RequestNoLookup', 'left=0, top=0, width=700, height=600, menubar=0,scrollbars=1')
			}
...
function catchbackspace() {
  ...
  if (e.keyCode==8){
    ...
    return false;
			...
			function OpenWinViewOrderOfTransfer(pTransferNo,pBranchID,ppvno)
			{				
				window.open(ServerName + App +  '/Finance/AccPay/ViewOrderOfTransfer.aspx?TransferNo=' + pTransferNo + '&BranchID=' + pBranchID+ '&PVNo=' + ppvno, 'AccPay', 'left=0, top=0, width=900, height=400, menubar=0,scrollbars=1')
			}
